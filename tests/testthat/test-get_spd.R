test_that("spd is returned", {
  ...
  expect_identical(names(spd)[1:2], c("pc7", "pc8"))
  expect_identical(nrow(unique(spd["hb2019"])), 14)
  expect_identical(nrow(unique(spd["hscp2019"])), 31)
  expect_identical(nrow(unique(spd["ca2019"])), 32)
  expect_identical(nrow(unique(spd["datazone2011"])), 6976)
})
