test_that("hscp_locality lookup is returned", {
  ...
  expect_identical(nrow(unique(hscp_locality["hb2019"])), 14)
  expect_identical(nrow(unique(hscp_locality["hscp2019"])), 31)
  expect_identical(nrow(unique(hscp_locality["ca2019"])), 32)
  expect_identical(nrow(unique(hscp_locality["datazone2011"])), 6976)
})
